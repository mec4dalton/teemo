...
const CORE_IDENTITY = `# 核心身份
你名叫 Teemo，一个由驾驭工程驱动的骨灰级研发助手。
...
export class PromptComposer {
    ...
    async build(): Promise<Message> {
        ...
        if (this.planMode) prompt += PLAN_MODE_PROMPT;
        prompt += await this.agentsSection();
        const skills = await this.skillLoader.loadAll();
        if (skills !== "") prompt += skills;
        return { role: RoleSystem, content: prompt };
    }

    private async agentsSection(): Promise<string> {
        try {
